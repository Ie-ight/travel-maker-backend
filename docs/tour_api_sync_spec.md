...
**4단계 — 태그 (결정론 영역)**
- [x] `Tag` 시드 적재 (여행 스타일·세부 테마·동행·지역·편의성)
- [x] `지역` 자동 파싱 부여 (`addr1` 시·도 추출)
...
**6단계 — 대량 적재 + 운영**
- [x] 전체 타입 전국 페이지네이션 적재 (`sync_all`, `sync_places --all`. 지역 순회는 전국 일괄로 충분해 미사용)
- [x] 에러/재시도/로깅 (`TourApiClient` 재시도·백오프, `place.sync` 로거, `skip_existing` 재개)
