...
from apps.place.services.place_sync import sync_all, sync_area
from apps.place.services.tour_api import TourApiError


class Command(BaseCommand):
    help = "한국관광공사 Tour API로 장소를 수집한다(소량 검증 또는 --all 대량 적재)."
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        ...
        parser.add_argument("--max-pages", type=int, default=None, help="--all 시 타입별 최대 페이지(미지정 시 끝까지)")
        parser.add_argument(
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if num_rows is None:
            num_rows = 1000 if is_all else 10
        try:
            if is_all:
                summary = sync_all(
        ...
        self.stdout.write(self.style.SUCCESS(f"수집 완료{mode}{' [전체 타입]' if is_all else ''}"))
