...
class TourApiClient:
    ...
    def _get(self, operation: str, params: dict[str, Any]) -> list[dict[str, Any]]:
        """재시도 래퍼. 일시 오류(타임아웃·5xx·"22")는 지수 백오프로 재시도, 치명 오류는 즉시 전파한다."""
        merged: dict[str, Any] = {
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if exc.code == "22":  # 트래픽 초과는 더 길게 쉬어준다
                    delay = max(delay, self.backoff_base * 8)
                logger.warning(
                    "%s 일시 오류, %.1fs 후 재시도(%d/%d): %s", operation, delay, attempt + 1, self.max_retries, exc
                )
                if delay:
                    time.sleep(delay)
        raise AssertionError("unreachable")  # 루프가 항상 return 또는 raise
    ...
    def _request_once(self, operation: str, url: str, merged: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        except requests.RequestException as exc:  # 타임아웃·연결·5xx → 일시 오류
            raise TourApiError(f"{operation} 호출 실패: {exc}", retryable=True) from exc
